...
\wde{3.3.2 A natural number system} A \textit{natural number system} is a set $N$ together with an element $z \in N$ and a function $\sigma \colon N \to N$, with the following property: for alal sets $X$, elements $a \in X$ and functions $r \colon X \to X$, there is a unique function $x \colon N \to X$ such that $x(z) = a$ and $x(\sigma(n)) = r(x(n))$ for all $n \in N$. In a diagram:
