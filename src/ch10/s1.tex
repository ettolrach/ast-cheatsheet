\wc{Recall:} $\mathbf{m} \leq \mathbf{n} \iff m \leq n$ and $\mathbf{m} \cong \mathbf{n} \iff m = n$.
\wde{no number, Infinite} A set is infinite if it is not finite.
\wl{Remark 10.1.1 Pigeonhole principle} Let $m, n \in \N$. If $m > n$ then there is no injection $\mathbf{m} \to \mathbf{n}$.
\wde{no number, Cardinality} The \textit{cardinality} of a finite set $X$, writen as $|X|$, is the unique natural number $n$ such that $X \cong \mathbf{n}$.
...
\wl{10.1.4} Let $X$ and $Y$ be sets. Then $X \leq Y$ if and only if $X$ is empty or there exists a surjection $Y \to X$.
