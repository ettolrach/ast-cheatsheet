\wde{n.n., Retraction} For sets $X$ and $Y$, a \textit{retraction} of a function $i \colon Y \to X$ is a function $f \colon X \to Y$ such that $f \circ i = \id_Y$.
...
\wl{5.5.8} Let $f \colon X \to Y$ be a function. Let $B \xhookrightarrow{j} Y$ be a subset such that $\im f \subseteq_Y B$. Then there is a unique function $f' \colon X \to B$ such taht $j(f'(x)) = f(x)$ for all $x \in X$. The function $f'$ is called the \textit{corestriction} of $f$ to $B$.
\wpr{5.5.10} Let $f \colon X \to Y$ be a function. Write $j$ for the inclusion $\im f \hookrightarrow Y$. Then there is a unique function $f' \colon X \to \im f$ such that the below-left triangle commutes, and $f'$ is surjective.
